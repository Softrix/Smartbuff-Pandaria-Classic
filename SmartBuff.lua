...
SMARTBUFF_DATE               = "090825";
SMARTBUFF_VERSION            = "r70." .. SMARTBUFF_DATE;
...
local cIgnoreClasses         = { 12, 13, 14, 15, 16, 19 };
...
local function SBGetSpecialization()
...
function SMARTBUFF_PlaySpashSound()
  PlaySoundFile(soundPath..Sounds[O.SBSoundSelection]);
end
...
local function CS()
  if (currentSpec == nil) then
    currentSpec = SBGetSpecialization();
  end
  if (currentSpec == nil) then
    currentSpec = 1;
...
function SMARTBUFF_OnEvent(self, event, ...)
  ...
  if event == "PLAYER_LOGIN" then
    ...
    -- beta had an issue where it forgot ui scale, this was temporary.. leaving this here just in case lol
--    SetCVar("uiScale", 0.65 );      -- this needs to be removed when bliz fixes ui scaling.
  end
  ...
  elseif (event == "PLAYER_TALENT_UPDATE") then
    ...
    if (currentSpec ~= SBGetSpecialization()) then
      currentSpec = SBGetSpecialization();
...
function SMARTBUFF_OnUpdate(self, elapsed)
  ...
  if (not isInit) then
    if (isLoaded and GetTime() > tAutoBuff + 0.5) then
      tAutoBuff = GetTime();
      local specID = SBGetSpecialization()
      if (specID) then
  ...
  if (UnitExists(unit) and UnitIsFriend("player", unit) and not UnitIsDeadOrGhost(unit) and not UnitIsCorpse(unit)
    ...
    for i, buffnS in pairs(B[CS()].Order) do
      ...
      if (bUsable and bs.EnableS and (cBuff.IDS ~= nil or SMARTBUFF_IsItem(cBuff.Type) or cBuff.Type == SMARTBUFF_CONST_TRACK)
		...
		-- bugfix : scroll was getting locked if zooming off and no mount prompt.
        if not O.ScrollWheelZooming and not O.ToggleMountedPrompt then
		    isPrompting = true;
		end
...
function SMARTBUFF_Options_Init(self)
  ...
  if (O.SBSoundSelection == nil) then O.SBSoundSelection = 10; end;
...
function SMARTBUFF_Options_OnShow()
  ...
  -- bugfix, slider not showing correct value on startup - cm 28/05/2025
  SmartBuffOptionsFrame_sldSounds:SetValue(O.SBSoundSelection);
